...
def filter_graph(Obj, cutoff=np.inf,n_neighbors=None,metric="euclidean"):
    if isinstance(Obj, nx.Graph):
        ...
        M = cdist(points, points,metric=metric)
        M = np.where(M <= cutoff, M, 0)
        H = nx.from_numpy_array(M)
        if n_neighbors!=None:
            H=knn_graph(nx.to_numpy_array(H),k=n_neighbors)
...
def compute_FRC(object D, float cutoff=np.inf,n_neighbors=None, int max_dim=1, mapped_nodes=False,metric="euclidean"):
    ...
    If D is not a dictionary, this parameter is ignored.

    Output: A dictionary whose keys are the dimensions and the values are dictionaries with the FRC for each cell."""
    global Neigh
...
def compute_FRC_node(object D, float cutoff=np.inf,n_neighbors=None, int max_dim=1, mapped_nodes=False,metric="euclidean"):
    ...
    Output: A dictionary whose keys are the dimensions and the values are dictionaries with the FRC for each node.
...
def compute_average_FRC(object D, float cutoff=np.inf, n_neighbors=None, int max_dim=1,metric="euclidean"):
    ...
    Output: A dictionary whose keys are the dimensions and the values are the average FRC for respective dimension."""
...
def compute_FRC_node_frequency(object D, float cutoff=np.inf,n_neighbors=None, int max_dim=1, mapped_nodes=False,metric="euclidean"):
...
def compute_FRC_frequency(object D, float cutoff=np.inf,n_neighbors=None, int max_dim=1,metric="euclidean"):
    """Computes the frequency of Forman-Ricci Curvature (FRC) values up to dimension dim from the object provided.
